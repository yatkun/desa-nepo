...
@section('konten')
    <section class="">
        <div class="flex items-center justify-between">
            <h3 class="font-semibold text-heading">Daftar Penduduk</h3>
            <a href="{{ route('penduduk.create') }} "
                class="px-3 py-2 text-sm text-white bg-green-600 rounded hover:bg-green-700">Tambah Penduduk</a>
        </div>
                                ...
                                @foreach ($penduduk as $item)
                                    <tr>
                                        ...
                                        <td class="px-12 py-4 font-medium whitespace-nowrap">
                                            <a href="{{ url('penduduk/edit') }}/{{ $item->id }}"
                                                class="text-primary hover:underline">{{ $item->nama }}</a>

                                        </td>
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <p class="text-gray-700">{{ $item->nik }}</p>
                                        </td>
                                        <td class="px-4 py-4 text-sm whitespace-nowrap">
                                            <div class="flex items-center">
                                                <p class="text-gray-700">{{ $item->pekerjaan }}</p>
                                            </div>
                                        </td>
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <p class="text-gray-700">{{ $item->nohp }}</p>
                                        </td>
