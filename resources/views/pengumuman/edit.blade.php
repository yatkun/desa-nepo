...
@section('konten')
    <h3 class="mb-3 font-semibold text-heading">Edit Pengumuman</h3>
    
        <div class="card" x-data="{ images: [] }">
            <div class="flex items-center justify-between pb-4 mb-3 border-b">
                <a href="{{ route('pengumuman') }}" class="inline-block px-3 py-2 text-sm text-white rounded bg-sky-500">Kembali</a>
                <a onclick="return confirm('Apakah Anda Yakin Menghapus Berita Ini?');" href="/pengumuman/hapus/{{$post->id}}" class="px-3 py-2 text-sm text-white rounded bg-rose-500">Hapus</a>
            ...
            <div class="mb-5">
                <div>
                    ...
                    <input autofocus class="w-full p-2 mb-4 bg-white border border-gray-300 outline-none title"
                        spellcheck="false" placeholder="Masukkan judul pengumuman" id="judul" name="judul" type="text" value="{{ $post->judul }}">
                </div>
                <div>
